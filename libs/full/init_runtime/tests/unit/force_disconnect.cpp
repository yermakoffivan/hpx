...
void test_calling_from_non_console_locality(std::uint32_t const locality_id)
{
    if (locality_id != 0)
    {
        ...
        // error_code overload
        {
            ...
            HPX_TEST(msg.find("HPX(invalid_status)") != std::string::npos ||
                msg.find("should be called on the console") !=
                    std::string::npos);
        }
...
void test_non_console_targets_non_console(std::uint32_t const locality_id)
{
    ...
    {
        if (id != hpx::naming::get_id_from_locality_id(0))
        {
            target = id;
            break;
        }
...
namespace {
    ...
    // Once a locality has been force-disconnected, parcels that are still in
    // flight to it (or dispatched to it afterwards, e.g. by the action-failure
    // checks below) are expected to fail to be sent. Since hpx_main() and main()
    // do not share a live call stack across the shutdown boundary, these are
    // kept in variables with static storage duration, mirroring finalize_time
    // below.
    hpx::spinlock disconnected_mtx;
    std::unordered_set<std::uint32_t> disconnected_localities;

    // Swallow parcel-write errors that are directed at localities we just
    // disconnected on purpose (these are expected and not actual test
    // failures), while still flagging any unexpected write failure to a
    // locality that is supposed to remain connected.
    bool parcel_write_handler(
        std::error_code const& ec, hpx::parcelset::parcel const& p)
    {
        if (!ec)
        {
            return false;    // keep processing this event
        }

        std::uint32_t const dest =
            hpx::naming::get_locality_id_from_gid(p.destination());

        {
            std::lock_guard<hpx::spinlock> l(disconnected_mtx);
            if (disconnected_localities.contains(dest))
            {
                // expected: ignore errors sending to removed localities
                return true;
            }
        }

        HPX_TEST(false);    // unexpected failure to a still-connected locality

        return true;    // suppress follow-on errors
    }
}    // namespace
    ...
    std::uint32_t const locality_id)
{
    ...
    // From this point on, any parcel that fails to be sent to this locality is
    // expected, since the locality has just been (intentionally) removed;
    // record it so that parcel_write_handler() knows to silently ignore such
    // failures instead of flagging them as unexpected errors.
    {
        std::lock_guard<hpx::spinlock> l(disconnected_mtx);
        disconnected_localities.insert(
            hpx::naming::get_locality_id_from_id(target));
    }
    ...
    std::uint32_t const locality_id, hpx::id_type const& second_target)
{
    ...
    // As above, parcel-write failures to this now-disconnected locality are
    // expected from here on.
    {
        std::lock_guard<hpx::spinlock> l(disconnected_mtx);
        disconnected_localities.insert(
            hpx::naming::get_locality_id_from_id(second_target));
    }
...
int hpx_main()
{
    ...
    hpx::set_config_entry("hpx.enable_fault_tolerance", "1");

    // set a parcel write handler that makes the parcellayer ignore failures
    // caused by disconnected localities
    //hpx::set_parcel_write_handler(&parcel_write_handler);
    ...
    // so far no disconnects have happened
    hpx::distributed::barrier::synchronize();

    auto const [fst, snd] = test_force_disconnect_removes_locality(locality_id);
    test_double_disconnect_is_idempotent(locality_id, fst);
    test_sequential_disconnects(locality_id, snd);
...
int main(int argc, char* argv[])
{
    ...
    HPX_TEST_LT(shutdown_seconds.count(), 30);
